...
const Register = () => {
  ...
  const handleRegister = () => {
    ...
    if (password == "") {
      ...
      if (!checkPassword) {
        setPasswordError(
          "Password should contain atleast 8 characters, 1 uppercase, 1 lowercase, 1 number and 1 symbol"
        );
        flag++;
        setLoading(false);
      }
      ...
      if (mode == "user") {
        fetch("http://localhost:4000/api/auth/register", {
          ...
          .then((data) => {
            console.log(data);
            setLoading(false);
            toast.success(username + " successfully registered as user!", {
              position: "top-center",
              autoClose: 5000,
        ...
        fetch("http://localhost:4000/api/auth/register", {
          ...
          .then((data) => {
            setLoading(false);
            console.log(data);
            toast.success(username + " successfully registered as seller!", {
              position: "top-center",
              autoClose: 5000,
              hideProgressBar: false,
              closeOnClick: true,
  ...
  return (
    <div className="w-full h-screen text-white bg-black">
      ...
      <div className="w-2/5 h-4/5 flex justify-center mx-auto content-center align-middle flex-col px-8 mb-2 pb-3 text-2xl text-center border-5 rounded-3xl border-green-500 shadow-2xl shadow-green-500/70 bg-blend-multiply text-">
        <div className="flex w-full h-80 text-4xl justify-start content-center flex-col pb-2 pt-12 login_heading hero glitch layers" data-text="REGISTER">
          <span>REGISTER</span>
        </div>
        <Radio.Group
          name="Register as"
          label="Register Mode"
          description="Select your Register mode"
          className="flex w-full h-30 justify-start content-center align-middle flex-col text-left pt-2 pb-2 mt-5"
          withAsterisk
        >
          <Radio.Group
            mt="xs"
        ...
        /> : <Button fullWidth variant="filled" onClick={handleRegister}>
            Register
          </Button>}
        </div>
        <p className="text-sm flex text-center justify-center content-center mb-4 pb-2">
